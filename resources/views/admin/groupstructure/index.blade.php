...
<div class="grid_12">
	...
	<br />
	<form action="{{ route('groupstructure.index') }}" method="post" id="frm1" name="frm1" enctype="multipart/form-data" >
		{{ csrf_field() }}
		<ol>
            <li>
                ...
                <br>
                <label></label>
                <img id="imagecontent2" src="{{ ((isset($item) && $item['groupstructure']->content2) ? image(''.$item['groupstructure']->content2, Config::get('app.directory.images')) : '') }}" alt="" style="max-width:700px"/>
                <input type="hidden" name="old_img" value="{{ (isset($item) ? url(''.$item['groupstructure']->content2) : '') }}" />
            </li>
			<li>
				<input type="submit" name="sendimg" value=" save " /> &nbsp;
				<input type="button" name="cancel" value=" back " onclick="location.replace('{{ route('groupstructure.index') }}')"/>
			</li>
		</ol>
