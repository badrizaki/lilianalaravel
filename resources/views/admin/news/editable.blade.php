...
@section('content')
<div class="grid_12">
	<h3>{{ ($id == 0 ? 'Add New' : 'Update') }} News</h3>
	<br />
	<form method="post" action="{{ url(route('news.index') . ($id == 0 ? '' : '/' . $id)) }}" enctype="multipart/form-data" class="editor">
		{{ csrf_field() }}
		@if($id != 0)
		...
		<div>
			...
			&nbsp;
			<button type="button" onClick="javascript:window.location.href = '{{ route('news.index') }}';return false;">Cancel</button>
		</div>
