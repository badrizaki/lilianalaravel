...
@section('content')
<div class="grid_12">
	<h3>{{ ($id == 0 ? 'Add New' : 'Update') }} Information Disclosure</h3>
	<br />
	<form method="post" action="{{ url(route('informationdisclosure.index') . ($id == 0 ? '' : '/' . $id)) }}" enctype="multipart/form-data" class="editor">
		{{ csrf_field() }}
		@if($id != 0)
		...
		<ul>
            ...
            <li>
                ...
                <br>
                @if (isset($item) && $item->pdfUrlInd != "")
                <object id="imagefile2" data="{{ (isset($item) ? url(''.$item->pdfUrlInd) : '') }}" type="application/pdf">
		...
		<div>
			...
			&nbsp;
			<button type="button" onClick="javascript:window.location.href = '{{ route('informationdisclosure.index') }}';return false;">Cancel</button>
		</div>
