...
class AnnualReports extends BaseController
{
    ...
    protected $redirectTo = 'annualreports'; // REDIRECT URL
    protected $redirectIndex = 'annualreports.index'; // View index
    protected $redirectEditable = 'annualreports.editable'; // vied add and update
    protected $redirectShow = 'annualreports.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['annualreports'] = \App\Models\TextBank::where('key', 'annualreports')->get()->first();
        return $list;
    }
    ...
    */
    ...
    {
        $item = new \App\Models\AnnualReports();
        $item = $item->select(['*']);
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('annualreports.index').'/'.$item->annualreportsId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('annualreports.index').'/'.$item->annualreportsId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->annualreportsId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$annualreportsId}}')
    ...
    {
        return \App\Models\AnnualReports::find($id);
    }
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'annualreports')->update($data);
        return Redirect::to("admin/annualreports");
    }
}