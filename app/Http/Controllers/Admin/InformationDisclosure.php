...
class InformationDisclosure extends BaseController
{
    protected $mainPage = 'investorrelations'; // mainpage
    protected $redirectTo = 'informationdisclosure'; // REDIRECT URL
    protected $redirectIndex = 'informationdisclosure.index'; // View index
    protected $redirectEditable = 'informationdisclosure.editable'; // vied add and update
    protected $redirectShow = 'informationdisclosure.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['informationdisclosure'] = \App\Models\TextBank::where('key', 'informationdisclosure')->get()->first();
        return $list;
    }
    ...
    */
    ...
    {
        $item = new \App\Models\InformationDisclosure(); 
        $item = $item->select(['*']);
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('informationdisclosure.index').'/'.$item->informationdisclosureId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('informationdisclosure.index').'/'.$item->informationdisclosureId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->informationdisclosureId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$informationdisclosureId}}')
    ...
    {
        return \App\Models\InformationDisclosure::find($id);
    }
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'informationdisclosure')->update($data);
        return Redirect::to("admin/informationdisclosure");
    }
}