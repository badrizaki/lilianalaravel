...
class Why extends BaseController
{
    protected $mainPage = 'profile'; // mainpage
    protected $redirectTo = 'why'; // REDIRECT URL
    protected $redirectIndex = 'why.index'; // View index
    protected $redirectEditable = 'why.editable'; // vied add and update
    protected $redirectShow = 'why.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['why'] = \App\Models\TextBank::where('key', 'why')->get()->first();
        return $list;
    }
    ...
    */
    protected function listAjax()
    {
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('why.index').'/'.$item->whyId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('why.index').'/'.$item->whyId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->whyId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$whyId}}')
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'why')->update($data);
        return Redirect::to("admin/why");
    }
}