...
class MeetingShareholders extends BaseController
{
    protected $mainPage = 'investorrelations'; // mainpage
    protected $redirectTo = 'meetingshareholders'; // REDIRECT URL
    protected $redirectIndex = 'meetingshareholders.index'; // View index
    protected $redirectEditable = 'meetingshareholders.editable'; // vied add and update
    protected $redirectShow = 'meetingshareholders.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['meetingshareholders'] = \App\Models\TextBank::where('key', 'generalMeetingShareholders')->get()->first();
        return $list;
    }
    ...
    */
    ...
    {
        $item = new \App\Models\MeetingShareholders(); 
        $item = $item->select(['*']);
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('meetingshareholders.index').'/'.$item->meetingshareholdersId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('meetingshareholders.index').'/'.$item->meetingshareholdersId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->meetingshareholdersId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$meetingshareholdersId}}')
    ...
    {
        return \App\Models\MeetingShareholders::find($id);
    }
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'generalMeetingShareholders')->update($data);
        return Redirect::to("admin/meetingshareholders");
    }
}