...
class SustainabilityReport extends BaseController
{
    protected $mainPage = 'corporategovernance'; // mainpage
    protected $redirectTo = 'sustainabilityreport'; // REDIRECT URL
    protected $redirectIndex = 'sustainabilityreport.index'; // View index
    protected $redirectEditable = 'sustainabilityreport.editable'; // vied add and update
    protected $redirectShow = 'sustainabilityreport.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['sustainabilityreport'] = \App\Models\TextBank::where('key', 'sustainabilityreport')->get()->first();
        return $list;
    }
    ...
    */
    ...
    {
        $item = new \App\Models\SustainabilityReport(); 
        $item = $item->select(['*']);
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('sustainabilityreport.index').'/'.$item->sustainabilityreportId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('sustainabilityreport.index').'/'.$item->sustainabilityreportId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->sustainabilityreportId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$sustainabilityreportId}}')
    ...
    {
        return \App\Models\SustainabilityReport::find($id);
    }
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'sustainabilityreport')->update($data);
        return Redirect::to("admin/sustainabilityreport");
    }
}