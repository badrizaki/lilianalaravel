...
class DailyReports extends BaseController
{
    ...
    protected $redirectTo = 'dailyreports'; // REDIRECT URL
    protected $redirectIndex = 'dailyreports.index'; // View index
    protected $redirectEditable = 'dailyreports.editable'; // vied add and update
    protected $redirectShow = 'dailyreports.show'; // vied add and update
    ...
    */
    ...
    {
        $list = [];
        $list['dailyreports'] = \App\Models\TextBank::where('key', 'dailyReports')->get()->first();
        return $list;
    }
    ...
    */
    ...
    {
        $item = new \App\Models\DailyReports(); 
        $item = $item->select(['*']);
        ...
        return Datatables::of($item)
            ...
            {
                $action = '';
                $action .= '<a href="'.route('dailyreports.index').'/'.$item->dailyreportsId.'/edit" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['edit'].'"></i> Edit</a>&nbsp;&nbsp;&nbsp;';
                // $action .= '<a href="'.route('dailyreports.index').'/'.$item->dailyreportsId.'" class="btn btn-xs btn-primary"><i class="'.$this->icon['tables']['detail'].'"></i> Detail</a>&nbsp;&nbsp;&nbsp;';
                $action .= '<a href="#" class="btn btn-xs btn-primary" onClick="listManager.delete(\''.$item->dailyreportsId.'\')"><i class="'.$this->icon['tables']['delete'].'"></i> Delete</a>';
                return $action;
            })
            ->editColumn('id', 'ID: {{$dailyreportsId}}')
    ...
    {
        return \App\Models\DailyReports::find($id);
    }
    ...
    public function storeMain(Request $request)
    {
        ...
        $item = \App\Models\TextBank::where('key', 'dailyreports')->update($data);
        return Redirect::to("admin/dailyreports");
    }
}